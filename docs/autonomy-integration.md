...
| 项 | 值 |
|---|---|
| 状态 | **v1.3 — 待 autonomy 方复查**（第 4 节只剩 1 条：A6.1 的事件模型要求；未知 project=硬失败、列表过滤、游标单调/永久保留均已确认） |
...
> 已关闭：~~仓库地址从哪来~~（autonomy 自行推导，第 3 节 ✅）· ~~列表按 project 过滤 + 行字段~~（已确认支持，A3 ✅）· ~~未知 project 的行为~~（**硬失败**，A2 ✅）· ~~`message_seq` 单调性 / 历史保留期~~（**跨重启单调 + 永久保留**，A6 ✅）。
