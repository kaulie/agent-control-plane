...
export const OPENAPI_ROUTE_META: Record<string, RouteMeta> = {
  ...
  "POST /api/tasks/{taskId}/messages": {
    ...
    responses: {
      202: { description: "已投递给执行方（`executor: true` + `messageId` / `queued`）" },
      400: { description: "参数非法 / 执行方只收文字（消息没有投递）" },
      404: { description: "任务不存在 / 执行方那边没有这条 task（没有投递）" },
