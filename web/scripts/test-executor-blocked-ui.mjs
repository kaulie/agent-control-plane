 ...
 * 4) 面板渲染：选项按 1..N 列、点一下预填（`data-prefill`），没有选项时不出现选项块，自由输入永远在；
 * 5) 预填总线：只填不发（空串不发、能退订）。
 *
...
const { onExecutorPrefill, requestExecutorPrefill } = await import("../src/executorPrefill.ts");
...
assert.ok(html.includes("blocked-option-idx") && html.includes(">1.<") && html.includes(">2.<"), "选项按 1..N 列");
assert.ok(html.includes(`data-prefill="Merge it now"`), "选项带 data-prefill（点一下填进输入框）");
assert.ok(html.includes("都不合适？直接写在下面的输入框里（你自己的意见）。"), "自由输入永远在");
...
// ---- 5) 预填总线：只填不发 ----
const got = [];
const off = onExecutorPrefill((text) => got.push(text));
requestExecutorPrefill("Merge it now");
requestExecutorPrefill("   ");
requestExecutorPrefill("  Ask for changes first  ");
off();
requestExecutorPrefill("after unsubscribe");
assert.deepEqual(got, ["Merge it now", "Ask for changes first"], "空串不发；退订后不再收到");
